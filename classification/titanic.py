...
def feature_select(df: pd.DataFrame) -> None:
    # Include Sex and Embarked one-hot columns as predictors
    embarked_cols = [col for col in df.columns if col.startswith('Embarked_')]
    predictors = df[['Age', 'SibSp', 'Fare', 'Sex'] + embarked_cols]
...
## Reads the csv file, drops any rows with missing values, and converts Sex and Embarked to numeric values. It is called by main.
