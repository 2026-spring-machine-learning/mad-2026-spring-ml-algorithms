...
def correlation_heatmap(df: pd.DataFrame, cutoff: float = 0.3) -> None:
    corr_matrix = df.corr()
...
def filter_features(df: pd.DataFrame) -> pd.DataFrame:
    filtered = df.drop(columns=['Teamwork', 'Adaptability', 'Leadership'])
    return filtered
...
def training_model(past_df: pd.DataFrame):
    ...
    correlation_heatmap(X, cutoff=0.3)
